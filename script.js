const mensajesAlejandra = [
    { titulo: "Carta #1: El inicio", mensaje: "1095 días desde que nos conocemos, y siento que apenas estamos empezando." },
    { titulo: "Carta #2: La constante", mensaje: "En un mundo lleno de variables, tú te has vuelto mi constante favorita." },
    { titulo: "Carta #3: La sonrisa", mensaje: "No sé qué fórmula usas, pero siempre logras hacerme sonreír." },
...
function crearCartas() {
    misCartas.forEach((carta, index) => {
        ...
        divCarta.classList.add("letter");
        divCarta.dataset.unlock = carta.fechaDesbloqueo; 

        divCarta.innerHTML = `
            <div class="locked-view" id="locked-${index}">
                <h3>Carta #${index + 1} en camino...</h3>
                <p class="timer" id="timer-${index}">Calculando tiempo...</p>
            </div>
            <div class="unlocked-view" id="unlocked-${index}" style="display: none;">
                <h3>${carta.titulo}</h3>
                <p>${carta.mensaje}</p>
            </div>
        `;
...
setInterval(() => {
    ...
    const cartasElementos = document.querySelectorAll('.letter');

    cartasElementos.forEach((carta, index) => {
        const fechaDesbloqueo = new Date(carta.dataset.unlock).getTime();
        const distancia = fechaDesbloqueo - ahora;

        const vistaBloqueada = document.getElementById(`locked-${index}`);
        const vistaDesbloqueada = document.getElementById(`unlocked-${index}`);
        ...
        if (distancia <= 0) {
            ...
            timerText.innerHTML = `Se abre en: <br>${dias}d ${horas}h ${minutos}m ${segundos}s`;
        }
    });
}, 1000);
